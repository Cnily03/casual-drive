...
! function () {
    ...
    let lockClick = false;
    document.querySelector("form#signup").addEventListener("keydown", function (e) {
        if (e.keyCode === 13 && !lockClick) {
            lockClick = true;
            document.querySelector("form#signup #submit").click();
        }
    })
    document.querySelector("form#signup").addEventListener("keyup", function (e) {
        lockClick = false;
    })
    // bind submit event
    const SUBMIT_BTN = document.querySelector("form#signup #submit");
    SUBMIT_BTN.addEventListener("click", function (e) {
        const username = document.querySelector("form#signup [name='username']").value;
        const password = document.querySelector("form#signup [name='password']").value;
        const repassword = document.querySelector("form#signup [name='repassword']").value;
        ...
        SUBMIT_BTN.innerHTML = BACKUP_INNER_HTML; // 注册中的按钮文字
        httpRequest.post({
            ...
            callback: xhr => {
                ...
                if (xhr.status === 200 && res.code === 0) {
                    ...
                    setTimeout(() => {
                        window.location.href = GetUrlValue("from") || "/account/signin";
                    }, 1000);
                } else if (res.code === -9) {
